...
class PrimitiveTransformer(nn.Module):
    ...
    def forward(self, xyzs, rgbs):
        ...
        xyzs = new_featuresd1.transpose(2, 3).contiguous()      #  B ,L , N, C
        ...
        point_one = point_feat.reshape(B, L, N, C)
        point_one = point_one.reshape(B*L, N, C)
        point_one = point_one.permute(0,2,1)
        point_one = F.adaptive_max_pool1d(point_one, (1)).reshape(B, L, C)
        ...
        # print(primitive_feature.shape)
        primitive_feature = torch.reshape(input=primitive_feature, shape=(B, L, 64, C))  # [B, L, 200, C]

        primitive_one = primitive_feature.reshape(B*L, 64, C)
        primitive_one = primitive_one.permute(0,2,1)
        primitive_one = F.adaptive_max_pool1d(primitive_one, (1)).reshape(B, L, C)

        return point_one, primitive_one
