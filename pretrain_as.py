...
from datasets.AS_pretrain import SegDataset
import models.AS_pptr_pretrain as Models
# import models_all.AS_p4_pretrain as Models   # change to p4
...
def main(args):
    ...
    dataset = SegDataset(root='/datasets/AS_data',root_complete='/datasets/AS_data_base', train=True)
    ...
    lr = args.lr
    optimizer = torch.optim.SGD(list(model.parameters())+list(forward_predictor.parameters())+list(back_predictor.parameters()), lr=lr, momentum=args.momentum, weight_decay=args.weight_decay)
...
def parse_args():
    ...
    parser.add_argument('-m', '--mask-ratio', default=0.75, type=int)
    parser.add_argument('-pn', '--primitive-num', default=2, type=int)
