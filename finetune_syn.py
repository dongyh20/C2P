...
from datasets.synthia_base import *
import models.synthia_base as Models
...
def main(args):
    ...
    if args.resume:
        ...
        # update_dict = {"module."+k: v for k, v in pre_state.items()}
        # update_dict = {k.replace("module.",""): v for k, v in pre_state.items()}
        # update_dict = {"module."+k : v for k, v in pre_state.items()}
        update_dict = {"module."+k: v for k, v in pre_state.items() if k.startswith("conv") or k.startswith("transformer.")} 
        for name in update_dict.keys():
            print(name)
        net_state_dict = model.state_dict()
        net_state_dict.update(update_dict)
        model.load_state_dict(net_state_dict)
        print(pre_state['transformer.layers.1.0.fn.fn.to_qkv.weight'])
        print(model.state_dict()['module.transformer.layers.1.0.fn.fn.to_qkv.weight'])
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/data/prNpz', help='data path')
    parser.add_argument('--data-train', default='/trainval_raw.txt', help='meta list for training')
    parser.add_argument('--data-eval', default='/test_raw.txt', help='meta list for test')
    parser.add_argument('--label-weight', default='/labelweights.npz', help='training label weights')
